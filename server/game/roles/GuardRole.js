...
module.exports = {
  ...
  handleAction: (game, player, action) => {
    if (game.phase !== PHASES.NIGHT_GUARD) return false;
    ...
    if (action.type === "protect") {
      ...
      if (
        ...
        action.targetId === nightManager.guardState.lastTargetId
      ) {
        console.log(
          `[NightManager] Guard tried to protect invalid target: ${action.targetId}`
        );
        return false;
      }
      nightManager.actions.guardTarget = action.targetId;
    } else if (action.type === "skip") {
      nightManager.actions.guardTarget = null;
    }
