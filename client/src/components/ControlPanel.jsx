...
export default function ControlPanel({
  ...
  const renderActions = () => {
    ...
    if (myStatus === "dead" && !isMyLastWords && !isMyHunterTurn) {
      return (
        <div className="mt-auto">
          <button
            className="btn-secondary opacity-50 cursor-not-allowed bg-transparent border-dashed"
            disabled
          >
            {t("you_are_dead")}
          </button>
      ...
      return (
        <div className="mt-auto p-4 bg-surface/20 rounded-[var(--radius-lg)] border border-border/50 backdrop-blur-sm text-center animate-in">
          ...
          <p className="text-sm text-ink/90 font-medium animate-pulse">
            {t("close_eyes", "Please close your eyes")}
          </p>
        </div>
      );
    }
        ...
        <div className="mt-auto space-y-3">
          ...
          {skipAction && (
            <button
              ...
              onClick={() => actions.onAction("skip", false)}
            >
              {t("do_nothing")}
            </button>
          )}
      ...
      if (isMeDying) {
        return (
          <div className="mt-auto text-center">
            ...
            <button className="btn-primary" onClick={actions.onEndSpeech}>
              {t("end_speech")}
            </button>
          </div>
        );
    ...
    if (phase === "DAY_ANNOUNCE" || phase === "DAY_ELIMINATION") {
      return (
        <div className="mt-auto text-center text-sm text-muted animate-pulse">
          {t("judge_speaking")}
        </div>
      );
    }
  ...
  if (onlyActions) {
    return (
      <div className="flex flex-col h-full justify-end">{renderActions()}</div>
    );
  }
    ...
    return (
      <div className="flex-grow flex flex-col justify-end font-medium text-xs list-none overflow-y-auto p-2 h-full scrollbar-hide space-y-1">
        {logs.slice(-5).map(
          (
            log,
            i // Only show last 5 logs for 'less is more' focus
          ) => (
            <div
              key={i}
