...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    if (storedRoom && storedPid) {
      ...
      const attemptRejoin = () => {
        console.log(
          `[Landing] Attempting auto-rejoin to ${storedRoom} with PID ${storedPid}`
        );
  ...
  if (isRejoining) {
    return (
      <RejoinLoading t={t} roomId={sessionStorage.getItem("werewolf_room")} />
    );
  }
  ...
  return (
    <div className="h-[100dvh] w-full flex flex-col items-center justify-center p-6 bg-zinc-950 text-ink relative overflow-hidden transition-colors duration-500">
      {/* Background Layers */}
      <div className="absolute inset-0 z-0 pointer-events-none">
        {/* Base Gloom Gradient */}
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_30%,#27272a_0%,#09090b_60%,#000000_100%)] opacity-80" />
